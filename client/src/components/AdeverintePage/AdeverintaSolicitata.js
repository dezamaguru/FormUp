...
const AdeverintaSolicitata = () => {
    ...
    const handleUploadAdeverintaSolicitata = async (e) => {
        e.preventDefault();
        if(!file)
            return alert('Incarca fisierul!');
        const formData = new FormData();
        formData.append('file', file);

        try{
            const res = await axiosPrivate.post(`/adeverinte/${id}/upload`, 
                formData,
                {
                    headers: {
    ...
    return (
        ...
        {auth?.type === 'student' && (
            <div>
                    ...
                    <p><strong>Tip Adeverință:</strong> {adeverinta.tip_adeverinta}</p>
                ...
                <div>
                        {adeverinta ? (
                            <>
                                ...
                                <form onSubmit={handleUploadAdeverintaSolicitata} style={{marginBottom: '20px'}} >
                                    <input
                                    type="file"
